...
sed -i '/upload_max_filesize/s/= *2M/=150M/' /etc/opt/remi/php${1}/php.ini
sed -i '/post_max_size/s/= *8M/=300M/' /etc/opt/remi/php${1}/php.ini
sed -i '/max_execution_time/s/= *30/=300/' /etc/opt/remi/php${1}/php.ini

echo "FcgidMaxRequestLen 13631488" >> /etc/httpd/conf/httpd.conf
	
chmod +x /usr/local/vesta/data/templates/web/httpd/Vesta-PHP${1}.sh
